...
router.post('/ajouter-sessions', async (request, response) =>{
    ...
    else if (presents == "undefined" || presents == "") {
        return response.status(500).json({
            msg: "Veuillez entrer sa présence"
        });
    }
    try{
...
router.post('/present', async(request,response) => {
    ...
    if(typeof studentId === 'undefined' &&
       typeof sessionId == 'undefined'){ return response.status(500).json({"msg": "Veuillez saisir un studentId/sessionId "}); }

    const session = await sessionModel.findOne({ "_id": sessionId });
    if (!session) {
        return response.status(500).json({"msg": " Aucun Resultat pour cette session : " + error});
    } 

    const student = await studentModel.findOne({ "_id": studentId });
    if (!student) {
        return response.status(500).json({"msg": " Aucun Resultat pour cet étudiant : " + error});
    } 

    try{       
        if(session.presents.includes(studentId)){
            return response.status(200).json({"msg": "L'étudiant "+student.lastname+" "+student.firstname+" était bien présent à ce cour"});
        }
        else {
            return response.status(200).json({"msg": "L'étudiant "+student.lastname+" "+student.firstname+" n'était pas présent à ce cour"});
         }
    }
    catch{
        return response.status(500).json({"msg": " Il y a une erreur : " + error});
    }
